...
- **新增 inject_ai_spirit_walker.py**：灵魂行者每 0.5s（SalvoTick）自动施法
  - 阶段1 驱散：检测我方单位诅咒(Bcrs)/减速(Bslo) buff，英雄优先，释放 Dispel Magic 到目标位置
  - 阶段2 灵魂链：检测我方单位无灵魂链(Bspl) buff，英雄优先，释放 Spirit Link
  - 注：本图 buff rawcode 非标准，诅咒=Bcrs(非Bcur)、灵魂链=Bspl(非Bslf)，均经运行时诊断扫描确认
  - 施法者选择：遍历 ospw 找蓝量 >= 75 的，没蓝或没 SW 则跳过
  - 不在 Salvo 齐射白名单（避免 smart 指令打断施法），移动由 Combat_AI 的 GroupPointOrderLocBJ 每 1s 触发
  - hook 方式：在 SalvoTick 的 endfunction 前插入 SW_TickForPlayer 调用（和 Kodo 一致）

### V51c BM 150 码就近攻击 + 灵魂行者军团调度修复 (2026-07-16)

- **BM NORMAL fallback 改为 150 码就近攻击**：新增 `FindNearestEnemyInRange(bm, enemyP, 150.0)`，NORMAL 状态只打 150 码内最近敌人，不做目标选择。HUNT/EXECUTE 保持原有目标挑选逻辑。根因：`FindLowestHpHero` 选到后排黑暗游侠，BM 被人墙挡住卡住
- **灵魂行者军团跟随**：在 V40 守卫之后为 ospw 加 `GroupPointOrderLocBJ "attack"` dispatch，每 1s 触发，不受 V40 守卫限制，不在 Salvo 里（不打断施法）
- **底座图纠正**：V51 起误用 `UD-decisive-multiplayer.w3x`，已纠正回 `UD-decisive-base.w3x`（V42-V49 一直使用的底座图）

### V51b BM 目标选择解耦 (2026-07-16)

- **BM 不再跟随 Salvo focus target**：NORMAL fallback 删除 `udg_aiml_FocusTarget1/2` 读取，改为独立 `FindLowestHpHero` 选目标
- **Salvo 仍跟随 BM 目标**：单向数据流 `BM -> udg_bm_Target1 -> Salvo`，带 anti-kite 800 码检查
- 根因：Salvo 每 0.5s 切目标，BM 每 0.1s 重下 attack，攻击动画不断被打断

### V51 强制打印 + 先知闪电链修复 (2026-07-16)

- **BM 强制攻击打印**：STRIKE 状态每 1s 打印目标（不受 DebugMode 控制），NORMAL 状态打印目标切换
- **先知(Ofar) 从 Salvo RANGED_HEROES 移除**：Ofar 每 0.5s 被 smart 指令打断闪电链施法前摇。移除后先知由 WC3 默认 AI 自行施放
...
| 9.5 | 灵魂行者施法（驱散+灵魂链） | `inject_ai_spirit_walker.py` | — |
...
| Timer | tick | 驱动模块 | 说明 |
|---|---|---|---|
| SalvoTick | 0.50s | 齐射 + 集火后撤 + 灵魂行者施法 | 避免打断攻击前摇；HP 下降检测需 0.5s 窗口；SW 复用此 tick，无需独立 timer |
...
- `'Ofar'`（先知）已从远程英雄白名单移除（V51：避免 smart 指令打断闪电链施法）
- BM 在 DASH/STRIKE 状态时，Salvo 跟随 BM 目标（单向，带 anti-kite 800 码检查）
- Anti-kite：BM 目标距 Army 质心 >800 码时不跟随，改为 PickSalvoTarget 选最近前排
...
```
两个都出现才算注入成功。
## 剑圣 AI 详情（inject_ai_blademaster.py，V51c 更新）
...
> **V51c 改动**：NORMAL fallback 不再使用 `FindLowestHpHero` 选目标（会选到后排英雄被人墙挡住）。改为 `FindNearestEnemyInRange(bm, enemyP, 150.0)` 只攻击 150 码内最近敌人，不做目标选择。HUNT 和 EXECUTE 保持原有目标挑选逻辑。
>
> **V51b 改动**：BM 不再跟随 Salvo 的 `udg_aiml_FocusTarget1/2`。Salvo 仍可跟随 BM 目标（单向），但 BM 独立选目标，不会被 Salvo 0.5s 切目标带跑导致攻击动画被打断。
...
## 灵魂行者 AI（inject_ai_spirit_walker.py，V51d 新增）

### 功能

灵魂行者每 0.5s（SalvoTick）自动施法，两个阶段按优先级执行：

| 阶段 | 检测条件 | 目标选择 | 施法 |
|------|---------|---------|------|
| 1 驱散 | 我方单位有诅咒(`Bcrs`)或减速(`Bslo`) | 英雄优先 | Dispel Magic -> 目标位置 |
| 2 灵魂链 | 我方单位无灵魂链(`Bspl`) buff | 英雄优先 | Spirit Link -> 目标单位 |

### 施法者选择

遍历所有 `ospw`（灵魂行者），找第一个蓝量 >= 75 的。没蓝或没 SW 则跳过等下个 tick。

### 自定义地图 buff rawcode 对照表

本图多个 buff rawcode 与标准 WC3 不同，必须用运行时诊断扫描确认：

| Buff | 标准 rawcode | 本图实际 | 确认方式 |
|------|-------------|---------|---------|
| 灵魂链 (Spirit Link) | Bslf | **Bspl** | [SW-DIAG] 运行时扫描 |
| 诅咒 (Curse) | Bcur | **Bcrs** | [SW-DIAG2] 运行时扫描 |
| 无敌 (Invulnerability) | Bvul | Bvul | BM EXECUTE 诊断扫描 |
| 减速 (Slow) | Bslo | 未确认 | 待验证 |

### 不打断施法设计

灵魂行者**不在 Salvo 齐射白名单**（`RANGED_TROOPS` / `RANGED_HEROES`），不会被每 0.5s 的 `smart` 指令打断施法。移动由 Combat_AI 的 `GroupPointOrderLocBJ "attack"` 每 1s 触发（V51c 新增的独立 dispatch，不受 V40 守卫限制）。

### 步骤注入位置

| 步骤 | 脚本 | 说明 |
|---|---|---|
| 9.5 | `inject_ai_spirit_walker.py` | 注入灵魂行者施法 AI，hook 到 SalvoTick |

### hook 方式

在 SalvoTick 的 `endfunction` 前插入 `Trig_AIML_SW_TickForPlayer` 调用（和 Kodo 一致），兼容其他 SalvoTick 修改。
...
wc3-ai-pipeline/
  ...
  inject_ai_blademaster.py    ← 剑圣 BM AI（V51c: 150码就近攻击 + 目标解耦）
  inject_ai_kodo.py             ← 科多兽吞噬后撤 AI
  inject_ai_spirit_walker.py    ← 灵魂行者施法 AI（驱散 + 灵魂链，V51d）
