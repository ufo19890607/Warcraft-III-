...
function Trig_AIML_TC_Stomp_Logic takes unit tc returns nothing
    ...
    if GetUnitState(tc, UNIT_STATE_MANA) < udg_aiml_StompManaCost then
        call DisplayTextToPlayer(Player(0), 0, 0, "[TC] mana too low, skip")
        call DestroyGroup(g)
        return
    endif
    set cx = GetUnitX(tc)
    ...
    call GroupClear(g)
    if heroNear then
        call DisplayTextToPlayer(Player(0), 0, 0, "[TC] WAR STOMP (hero nearby!)")
    ...
    if count >= udg_aiml_StompMinEnemies then
        call DisplayTextToPlayer(Player(0), 0, 0, "[TC] WAR STOMP (enemies=" + I2S(count) + ")")
        call IssueImmediateOrderById(tc, 852127)
    else
        call DisplayTextToPlayer(Player(0), 0, 0, "[TC] wait (enemies=" + I2S(count) + ", need=" + I2S(udg_aiml_StompMinEnemies) + ")")
    endif
    call DestroyGroup(g)
endfunction
