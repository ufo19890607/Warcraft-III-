...
RANGED_HEROES = [
    # Heroes removed from salvo to avoid conflicting with skill casts
    # They will use their own combat AI dispatch (hex, chainlightning, etc.)
]
...
function Trig_AIML_SalvoForPlayer takes player p, player ep, integer focusSlot returns nothing
    local unit picked
    local boolean dbg = udg_aiml_DebugMode
    ...
    // Issue attack orders to ranged group
    if dbg then
        call DisplayTextToForce(GetPlayersAll(), "[AIML] FIRE on " + GetUnitName(picked) + " rangedN=" + I2S(udg_aiml_SalvoRangedCount))
    endif
...
def inject(in_path, out_path):
    ...
    # 4.5) [V43] Shockwave → Stomp 防呆: WC3 native AI may still order Otch to cast shockwave
    # even after we replaced AOsw→AOws in SelectHeroSkill. Replace any order-string "shockwave"
    # with "stomp" so TC uses War Stomp instead, regardless of which AI path triggers it.
    sw_count = src.count('"shockwave"')
    if sw_count > 0:
        src = src.replace('"shockwave"', '"stomp"')
        print(f"replaced \"shockwave\" -> \"stomp\": {sw_count} occurrences")
    else:
        print("no \"shockwave\" order-string found (clean)")
