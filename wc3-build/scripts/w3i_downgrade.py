...
def parse_v31(data: bytes):
    ...
    # we won't deeply parse rnd unit / item tables — for this map they're 0/empty
    rnd_unit_remaining = info['rndUnitCount']
    info['rndUnitsRaw']       = b.read(0)  # placeholder if 0

    info['rndItemCount']      = u32(b) if rnd_unit_remaining == 0 else 0  # safe guess
    if rnd_unit_remaining != 0:
        # not handling complex case for this map
        raise RuntimeError("random unit tables present - need full parser")
    info['rndItemRaw']        = b.read(0)
